Feature: Book Management- Create a Book by Admin
  ...
  Scenario: Unauthorized user attempts to create a book with invalid username
    Given the library API is running for book creation using admin
    And the API endpoint is "/books"
    And the user is authenticated as "invalidAdmin" with password "password"
    When I send a POST request to create a new book
  ...
  Scenario: Unauthorized user attempts to create a book with invalid password
    Given the library API is running for book creation using admin
    And the API endpoint is "/books"
    And the user is authenticated as "admin" with password "wrongPassword"
    When I send a POST request to create a new book
    Then the response status code should be 401
